 ...
 */
package com.ghosthuntar.mgr.downloader;
...
import com.ghosthuntar.MixContext;
import com.ghosthuntar.GameView;
import com.ghosthuntar.data.convert.DataConvertor;
import com.ghosthuntar.mgr.HttpTools;
...
class DownloadMgrImpl implements Runnable, DownloadManager {
	...
	public String submitJob(DownloadRequest job) {
		...
		if (job != null && job.getSource().isWellFormed()) {
			...
			if (!todoList.contains(job)) {
				...
				todoList.add(mJob);
				Log.i(GameView.TAG, "Submitted " + job.toString());
	...
	public void switchOn() {
		if (DownloadManagerState.OffLine.equals(getState())){
			...
			Log.i(GameView.TAG, "DownloadManager already started");
		}
	}
